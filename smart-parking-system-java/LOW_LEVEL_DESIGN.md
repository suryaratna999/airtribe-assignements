...
----------------------
1. Add a Spring Boot REST wrapper around the `ParkingService` and wire JPA repositories to persist slots and sessions.
2. Implement DB migrations (Flyway/Liquibase) using the DDL above.
3. Add integration tests using H2 and add a CI job to run them.
4. Add metrics (Micrometer) and a health endpoint.

Author and contact
------------------
Engineering Notes — prepared for the implementation team. If you want, I can:
- Implement the Spring Boot REST layer and persistence wiring next, or
- Add concurrency stress-tests and a small load script.
